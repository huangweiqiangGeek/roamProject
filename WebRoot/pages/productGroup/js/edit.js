...
function sortMethod(classname){
	$("."+classname).each(function(index,element){
		$(this).children("td:first-child").text(index+2);
		$(this).attr("id",$(this).attr("id").replace(/[\d]+/,index+2));
		var href=$(this).children("td:last-child").find("a").attr("href").replace(/[\d]+/,index+2);
...
function addBusinessTypeDIV(){
	...
	"</tr>"+
		...
		"<tr>"+
			...
			"<td><input class='pro'></td>"+
			"<td><a href='javascript:addMethod("+selectTypeID[i]+")' class='btn headerBtn' style='width:40px'>添加</a></td>"+
		"</tr>"+
		"</table>";
