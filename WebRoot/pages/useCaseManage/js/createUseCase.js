 ...
 * 初始化左边的树形数据
***/
var trHMmax=0 , flag1=0 , flag2=0; var proAways = 1; var pron = [];
//function searchPrd(){
//	onClickTree();
//}
	...
	function getUseCase(id){
		$.ajax({
			...
			success: function (result) {
				...
				$("input[name='id']").val(jsonstr.id);
				$("#executeNum").val(jsonstr.executeNum);
...
function initUCaseBaseMe(jsonstr){
	$("#productId").val(jsonstr.productId);
	$("#ticket").html('');
...
$("#resultName").val(jsonstr.resultName);
/*$("#expDetail").html('');*/
	...
	$("#haveSource").combobox({
		onChange: function (n,o) {
			if(n==0){
				...
				addProduct();
				//$("#explorer").html('<tr class="trHMs selVal" id='+trHMmax+'><td><select id="txtProIdInit'+trHMmax+'" name="txtProIdInit" class="combo-text validatebox-text" style="width:120px;"></select></td><td><input type="type" id="txtProInitVal" name="txtProInitVal" value="" style="width: 160px"></td><td><select name="setAssg" class="sel"><option value="0" selected>本月套餐</option><option value="1">上月结转</option><option value="2">套内与结转</option></select></td><td><span class="add1 delete"><a onclick="removeHM('+trHMmax+')"><i></i>删除</a></span></td></tr>');
			}else{
				$('#Source').val('-1');
			}
			if(n=0){
				...
				onLoadSuccess: function () { //加载完成后,设置选中第一项  
					...
					$.each(val, function(p1, p2) {
					     ...
					     for (var item in this) {  
								if (item == 'id') { 
									$('#task').combobox('select',this[item]);
								}  
							} 
					});
				}
...
function addProductLis(){
	var task_id=$("#proID").val();
	//初始化右边的表单
	showWin("userCaseTreeController/taskGroupView?task_id="+task_id,"新增任务下的产品组",800,450);
	 $("#tb").css("display","none");
}
...
function saveMethod(){
	...
	if($("#resourceJson").val()){
		...
		for(var i = 0; i<sel.length;i++){
		 ...
		 if($("#productId").val()==""){
			 $("#productId").val(resourceJson.pro[0].proName);
		 }
...
function loadExpect(){
	...
	var h='<thead><tr><th  width="180" class="head" style="text-align: center;">产品</th><th  width="180" style="text-align: center;">号码</th><th  width="180" style="text-align: center;">资源类型</th>';
	//拼接为HTML
	...
	if(proAway==1){
		 ...
		 $(dataJson).each(function(i,o){
				 ...
				 if(i==0){
					 ...
					 var h='';
					 h+='<tr class="trHMs selVal" id="'+i+'">';
						h+='<td><select id="txtProIdInit'+i+'" name="txtProIdInit" class="combo-text validatebox-text" style="width:120px;"></select>'//proName
